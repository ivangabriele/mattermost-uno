(function () {
  ...
  async function loop() {
    ...
    const $newTheme = document.querySelector("link.code_theme"); // If the theme has changed:
    ...
    $posts.forEach($post => {
      ...
      if ($post.classList.contains("post--comment")) {
        ...
        if (postsReplies.includes($post.id)) return; // We can't link a post reply whose root post is not visible:
      ...
      const $rootPostCounter = $post.querySelector(".comment-count"); // Id the counter node doesn't exist, it means that this root post doesn't have any reply.
      // We can ignore it:

      if ($rootPostCounter === null) return;
      const rootPostIndex = findPostIndexWithId($post.id); // If this root post is already in the list, we just need to update its counter:
