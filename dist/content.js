(function () {
  ...
  function isColorLight(color) {
    ...
    let r;
    let g;
    let b; // Check the format of the color, HEX or RGB?
    ...
    const hsp = Math.sqrt(0.299 * (r * r) + 0.587 * (g * g) + 0.114 * (b * b)); // Using the HSP value, we can determine whether the color is light or dark:

    return hsp > 127.5;
  }

  function stripHtmlTags(source) {
    return source.replace(/(.*)<span data-emoticon="([^"]+)">(.*)/g, "$1:$2:<span>$3").replace(/<[^>]*>/g, "");
  }

  function themize(source) {
    return source.replace(/[^\s\w-]/g, " ").replace(/\s+/g, " ").trim();
  }
  ...
  function updateCounters(rootPostsWithReplies) {
    rootPostsWithReplies.forEach(({
      $button,
      $node,
      ...
      $counter.addEventListener("click", () => $button.click()); // const $counterAuthors = document.createElement("div");
  ...
  const findPostIndexWithTheme = theme => findIndex(propEq("theme", theme))(rootPostsWithReplies); // If the location path has changed:
  ...
  async function loop() {
    ...
    while ($posts.length === 0 && retries-- > 0) {
      $posts = document.querySelectorAll(".post-list-holder-by-time .post"); // eslint-disable-next-line no-await-in-loop
    ...
    const lastPostId = $posts[$posts.length - 1].id; // If the location path changed and the first or last post id is the same,
    ...
    const $theme = document.querySelector("link.code_theme"); // If the theme has changed:
    ...
    if ($theme !== null && $theme.href !== themeHref) {
      ...
      if ($appContent !== null) {
        const appContentBackgroundColor = window.getComputedStyle($appContent, null).getPropertyValue("background-color");
    ...
    const $newPostList = document.querySelector(".post-list-holder-by-time"); // If the post list node has changed:
    ...
    $posts.forEach($post => {
      ...
      if ($post.classList.contains("post--comment")) {
        ...
        if (!parentRootPost.authors.includes($postUserPicture.src)) {
          ...
          if (parentRootPost.authors.length > parentRootPost.count || parentRootPost.authors.length > 5) {
  ...
  if ($mattermostMeta !== null && $mattermostMeta.content === "Mattermost") {
    ...
    connection.postMessage({
      value: CHANNEL_MESSAGE.CONTENT_IS_MATTERMOST
    });
    loop();
  }
