(function () {
  ...
  async function loop() {
    ...
    const lastPostId = $posts[$posts.length - 1].id; // If the posts list hasn't changed:
    ...
    $posts.forEach($post => {
      ...
      if ($post.classList.contains("post--comment")) {
        if (findPostIndexWithId($post.id) !== -1) return; // const $postUserPicture = $post.querySelector("img.more-modal__image");
        ...
        const parentRootPost = rootPostsWithReplies[rootPostIndex]; // if (!parentRootPost.authors.includes($postUserPicture.src)) {
